...
export async function registerRoutes(app: Express) {
  ...
  app.post("/api/events", async (req, res) => {
    try {
      ...
      const eventData = {
        ...
        availableSeats: Number(req.body.totalSeats) || 96,
  ...
  app.patch("/api/events/:id", async (req, res) => {
    try {
      ...
      if (req.body.date && originalEvent) {
        ...
        if (
          sentDate.getHours() === 0 &&
          sentDate.getMinutes() === 0 &&
          sentDate.getSeconds() === 0 &&
          ...
          console.log(
            `Preserving original time: ${originalDate.toISOString()} -> ${preservedDate.toISOString()}`,
          );
        }
      }
    ...
    try {
      ...
      try {
        ...
        if (isTestPayment) {
          ...
          stripeInstance = await getStripeForPaymentVerified(
            booking.stripePaymentId,
          );
        ...
        console.log(
          `📊 Using Stripe instance: ${isTestPayment ? "TEST" : "LIVE"} mode (${stripeInstance ? "initialized" : "NOT initialized"});`,
        );
