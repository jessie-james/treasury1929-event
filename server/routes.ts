...
// Initialize Stripe with Treasury 1929 keys - prioritize Treasury account (regular key, not NEW)
const stripeSecretKey = process.env.STRIPE_SECRET_KEY_TREASURY || process.env.STRIPE_SECRET_KEY || process.env.STRIPE_SECRET_KEY_NEW;
if (!stripeSecretKey) {
...
function initializeStripe() {
  ...
  try {
    if (stripeSecretKey) {
      ...
      const keySource = process.env.STRIPE_SECRET_KEY_TREASURY ? "TREASURY" : process.env.STRIPE_SECRET_KEY ? "TREASURY_REGULAR" : "SAHUARO_NEW";
