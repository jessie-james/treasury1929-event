...
export function EventForm({ event, onClose }: Props) {
  ...
  // For create mode, only fetch after user changes the default venue selection
  const initialSelectedVenueIdRef = useRef<number | undefined>(undefined);
  const [shouldFetchVenueLayout, setShouldFetchVenueLayout] =
    useState<boolean>(false);
  if (initialSelectedVenueIdRef.current === undefined) {
    initialSelectedVenueIdRef.current = selectedVenueId;
  }
  useEffect(() => {
    if (!isEditing && selectedVenueId !== initialSelectedVenueIdRef.current) {
      setShouldFetchVenueLayout(true);
    }
  ...
  const { data: venueLayout } = useQuery({
    ...
    enabled: !!selectedVenueId && (isEditing || shouldFetchVenueLayout),
    throwOnError: false,
  });
  ...
  useEffect(() => {
    if (
      Array.isArray(currentEventFoodOptions) &&
      currentEventFoodOptions.length > 0
    ) {
      const selectedIds = Array.isArray(currentEventFoodOptions)
        ? currentEventFoodOptions.map((option: any) => option.id)
        : [];
  ...
  const handleImageUpload = async (file: File) => {
    ...
    try {
      ...
      if (
        ...
        console.error(
          "Server returned HTML/XML instead of JSON:",
          responseText,
        );
      ...
      if (!response.ok) {
        throw new Error(
          data.message || "Upload failed - server returned an error",
        );
      }
    ...
    onSuccess: async (response) => {
      ...
      if (selectedFoodOptions.length > 0) {
        try {
          await apiRequest(
            "PUT",
            `/api/events/${savedEvent.id || event?.id}/food-options`,
            {
      ...
      queryClient.invalidateQueries({
        queryKey: [`/api/events/${savedEvent.id || event?.id}/food-options`],
      });
      toast({
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>
          {event ? "Edit Event" : "Create Event test this is clean room"}
        </CardTitle>
      </CardHeader>
      <CardContent>
        <Form {...form}>
          ...
          >
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="full">
                        Full Event (with venue/table selection)
                      </SelectItem>
                      <SelectItem value="ticket-only">
                        Ticket-Only Event (simple ticket purchase)
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <FormDescription>
                    Full events include venue layouts and table selection.
                    Ticket-only events are simple quantity-based purchases.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
                  ...
                  render={({ field }) => {
                    ...
                    return (
                      <FormItem>
                        ...
                        <FormDescription>
                          Custom price display text. Leave empty to use default:{" "}
              ...
              render={({ field }) => (
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                  ...
                  <div className="space-y-1 leading-none">
                    ...
                    <FormDescription>
                      Whether this event is active and should be displayed to
                      customers
                    </FormDescription>
                  </div>
                </FormItem>
            ...
            {/* Artists Section */}
            <EventArtists eventId={event?.id} isEditing={!!event} />
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Select which food options to serve at this event. Only selected
              ...
              {allFoodOptions.length > 0 ? (
                <Tabs defaultValue="salad" className="w-full">
                  ...
                  {["salad", "entree", "dessert"].map((type) => {
                    const selectedForThisType = selectedFoodOptions.filter(
                      (id) => {
                        const option = allFoodOptions.find(
                          (opt) => opt.id === id,
                        );
                      ...
                      >
                        {selectedForThisType >= 3 && (
                          <Alert>
                            ...
                            <AlertDescription>
                              <strong>Maximum reached:</strong> You can only
                              select up to 3 {type} options per event.
                            </AlertDescription>
                          </Alert>
                        )}
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Create food options first in the Food management section
                  </p>
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm text-muted-foreground">
                Select which wine and beverage options to serve at this event.
                Only selected items will be available to guests.
              </p>

              {allFoodOptions.filter((option) => option.type === "wine_bottle")
                .length > 0 ? (
                <div className="grid gap-3">
                  {allFoodOptions
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    Create wine options first in the Beverage management section
                  </p>
                </div>
              )}
            </div>
