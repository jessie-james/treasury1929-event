...
console.log(
  `🔑 Live key available: ${stripeLiveKey ? "YES" : "NO"} ${stripeLiveKey ? "(" + stripeLiveKey.substring(0, 7) + "...)" : ""}`,
);
console.log(
  `🔑 Test key available: ${stripeTestKey ? "YES" : "NO"} ${stripeTestKey ? "(" + stripeTestKey.substring(0, 7) + "...)" : ""}`,
);

if (stripeLiveKey) {
  const isTestKey = stripeLiveKey.startsWith("sk_test_");
  console.log(
    `Primary Stripe key detected as: ${isTestKey ? "TEST" : "LIVE"} (${stripeLiveKey.substring(0, 7)}...)`,
  );
}

if (!stripeLiveKey) {
  console.error(
    "STRIPE_SECRET_KEY_TREASURY, STRIPE_SECRET_KEY_NEW or STRIPE_SECRET_KEY environment variable not set. Stripe payments will not work.",
  );
}
...
function initializeStripe() {
  ...
  try {
    ...
    if (stripeLiveKey) {
      ...
      const keySource = process.env.STRIPE_SECRET_KEY_TREASURY
        ? "TREASURY"
        : process.env.STRIPE_SECRET_KEY_NEW
          ? "TREASURY_NEW"
          : "SAHUARO_OLD";
      console.log(
        `Using Stripe key with prefix: ${keyPrefix}... (${keySource})`,
      );
      ...
      if (isActuallyTestKey) {
        console.log(
          "Primary key is test mode - using same instance for test operations",
        );
        stripeTest = stripe;
      } else {
        ...
        if (actualTestKey && actualTestKey.startsWith("sk_test_")) {
          ...
          console.log(
            `✓ Stripe TEST instance created with test key: ${actualTestKey.substring(0, 12)}...`,
          );
        } else {
          console.log(
            "⚠️ STRIPE_SECRET_KEY does not contain test key - using fallback",
          );
      ...
      console.error(
        "× Cannot initialize Stripe: STRIPE_SECRET_KEY_NEW or STRIPE_SECRET_KEY is missing",
      );
      return false;
    }
  } catch (error) {
    console.error(
      `× Failed to initialize Stripe (attempt ${stripeInitAttempt}):`,
      error,
    );
    return false;
  }
...
(async () => {
  try {
    ...
    if (!success) {
      console.log(
        "First Stripe initialization failed, retrying after delay...",
      );
...
const fileFilter = (
  ...
  console.log(
    `Receiving file upload: ${file.originalname}, MIME type: ${file.mimetype}`,
  );
  ...
  if (allowedMimeTypes.includes(file.mimetype)) {
    ...
    const error = new Error(
      `Invalid file type: ${file.mimetype}. Only JPEG, PNG, GIF, WebP, SVG and BMP images are allowed.`,
    );
...
async function clearAllBookings() {
  try {
    ...
    return {
      success: true,
      message: "All bookings cleared and event seats reset",
    };
  } catch (error) {
...
export async function registerRoutes(app: Express) {
  ...
  app.get("/api/debug/phantom-layouts/:eventId", async (req, res) => {
    try {
      ...
      console.log(
        `📋 Found ${allEventVenues.length} event-venue records:`,
        allEventVenues,
      );
      ...
      console.log(
        `🏢 Actual venues found: ${actualVenues.length}`,
        actualVenues.map((v) => ({ id: v.id, name: v.name })),
      );

      // Identify orphaned event-venue records
      const actualVenueIds = actualVenues.map((v) => v.id);
      const orphanedEventVenues = allEventVenues.filter(
        (ev) => !actualVenueIds.includes(ev.venueId),
      );

      console.log(
        `👻 PHANTOM LAYOUTS FOUND: ${orphanedEventVenues.length}`,
        orphanedEventVenues,
      );

      // Check for suspicious display names
      const suspiciousEventVenues = allEventVenues.filter(
        (ev) =>
      ...
      console.log(
        `⚠️ SUSPICIOUS LAYOUTS: ${suspiciousEventVenues.length}`,
        suspiciousEventVenues,
      );
  ...
  app.post("/api/debug/cleanup-phantoms/:eventId", async (req, res) => {
    try {
      ...
      console.log(
        `🧹 PHANTOM CLEANUP for Event ${eventId} (dryRun: ${dryRun})`,
      );
      ...
      const actualVenueIds = actualVenues.map((v) => v.id);
      const phantomRecords = allEventVenues.filter(
        (ev) => !actualVenueIds.includes(ev.venueId),
      );

      // Also find suspicious display names
      const suspiciousRecords = allEventVenues.filter(
        (ev) =>
          ev.displayName.length < 3 ||
          ev.displayName.toLowerCase() === "s" ||
      ...
      const recordsToDelete = Array.from(
        new Set([...phantomRecords, ...suspiciousRecords]),
      );

      console.log(
        `🎯 Found ${recordsToDelete.length} phantom records to delete:`,
  ...
  app.post("/api/debug/deduplicate-tables/:venueId", async (req, res) => {
    try {
      ...
      console.log(
        `🔧 TABLE DEDUPLICATION for Venue ${venueId} (dryRun: ${dryRun})`,
      );
      ...
      console.log(
        `📊 Found ${allTables.length} total tables, ${bookedTableIds.size} have bookings`,
      );

      // Group by table number to find duplicates
      const tableGroups = allTables.reduce(
        (acc, table) => {
      ...
      for (const [tableNumber, duplicates] of Object.entries(tableGroups)) {
        if (duplicates.length > 1) {
          console.log(
            `🔍 Table ${tableNumber}: ${duplicates.length} duplicates`,
          );

          // Find the best table to keep (prefer one with bookings, then oldest)
          const bookedDuplicates = duplicates.filter((t) =>
            bookedTableIds.has(t.id),
          );
          const unbookedDuplicates = duplicates.filter(
          ...
          deduplicationPlan.push({
            ...
            keeping: {
              id: tableToKeep.id,
              hasBookings: bookedTableIds.has(tableToKeep.id),
            },
            deleting: toDelete.map((t) => ({
              id: t.id,
              hasBookings: bookedTableIds.has(t.id),
            })),
          });
      ...
      console.log(
        `🎯 Deduplication plan: ${deduplicationPlan.length} table numbers affected`,
      );
      console.log(`🗑️ Tables to delete: ${tablesToDelete.length}`);

      // Safety check: ensure no booked tables are deleted
      const bookedTablesToDelete = tablesToDelete.filter((id) =>
        bookedTableIds.has(id),
      );
      if (bookedTablesToDelete.length > 0) {
        console.error(
          `❌ SAFETY VIOLATION: ${bookedTablesToDelete.length} booked tables would be deleted!`,
        );
  ...
  app.post("/create-booking-final", async (req, res) => {
    ...
    try {
      ...
      console.log(
        "🟢 CREATING BOOKING (FINAL ATTEMPT):",
        JSON.stringify(bookingData, null, 2),
      );
      const newBooking = await storage.createBooking(bookingData);
      console.log(
        "🟢 FINAL BOOKING SUCCESS:",
        (newBooking as any)?.id || "booking created",
      );
      ...
      try {
        ...
        console.error(
          "Email notification failed (booking still successful):",
          emailError,
        );
      }
    ...
    ws.on("message", (message) => {
      try {
        ...
        // Handle venue subscriptions for layout editor
          ...
          data.type === "subscribe_venue" &&
          typeof data.venueId === "number"
        ) {
  ...
  const broadcastCheckInUpdate = async (eventId: number) => {
    try {
      ...
      console.error(
        `Error broadcasting check-in update for event ${eventId}:`,
        error,
      );
    }
  };
    ...
    try {
      ...
      const eventData = {
        ...
        totalTables:
          ...
          Number(req.body.availableTables) ||
          32,
        availableTables: Number(req.body.availableTables) || 32,
        availableSeats:
          Number(req.body.availableSeats) || Number(req.body.totalSeats) || 96,
        ...
        // EVENT OPTIONS
        includeFoodService:
          req.body.includeFoodService !== undefined
            ? req.body.includeFoodService
            : true,
        includeBeverages:
          req.body.includeBeverages !== undefined
            ? req.body.includeBeverages
            : true,
        includeAlcohol:
          req.body.includeAlcohol !== undefined
            ? req.body.includeAlcohol
            : true,
  ...
  app.patch("/api/events/:id", async (req, res) => {
    try {
      ...
      if (updateData.venueId) {
        try {
          const venueLayout = await (storage as any).getVenueLayout(
            updateData.venueId,
          );
          if (venueLayout?.tables) {
            const totalSeats = venueLayout.tables.reduce(
            ...
            console.log(
              `Event ${id} venue changed to ${updateData.venueId}: ${totalSeats} seats from ${totalTables} tables`,
            );
          }
        } catch (venueError) {
          console.log(
            `Could not fetch venue layout for venue ${updateData.venueId}:`,
            venueError,
          );
        }
      }
      ...
      for (const key of Object.keys(updateData)) {
        if (
          JSON.stringify(originalEvent[key as keyof typeof originalEvent]) !==
      ...
      if (event.venueId) {
        try {
          ...
          if (venue) {
            ...
            console.log(
              `Tables found for venue ${event.venueId}:`,
              tables.length,
            );
            ...
            console.log(
              `Stages found for venue ${event.venueId}:`,
              stages.length,
            );
  ...
  app.get("/api/events/:eventId/availability", async (req, res) => {
    ...
    try {
      ...
      if (cached && now - cached.timestamp < ENDPOINT_CACHE_TTL) {
        ...
        console.log(
          `[PERFORMANCE] Availability cache HIT for event ${eventId} - ${Date.now() - startTime}ms`,
        );
      ...
      const availability =
        await AvailabilitySync.getRealTimeAvailability(eventId);
      ...
      if (duration > 100) {
        console.log(
          `[PERFORMANCE] Slow availability request for event ${eventId} - ${duration}ms`,
        );
      }
  ...
  app.post("/api/events/:eventId/venues", async (req, res) => {
    try {
      ...
      if (existingVenues.length >= 2) {
        return res
          .status(400)
          .json({ message: "Maximum 2 venues allowed per event" });
      }

      // Check if venue already exists for this event
      const venueExists = existingVenues.some(
        (ev) => ev.venueId === venueData.venueId,
      );
      if (venueExists) {
        return res
          .status(400)
          .json({ message: "Venue already added to this event" });
      }

      // Validate display name uniqueness within the event
      const nameExists = existingVenues.some(
        (ev) => ev.displayName === venueData.displayName,
      );
      if (nameExists) {
        return res
          .status(400)
          .json({ message: "Display name already used for this event" });
      }
  ...
  app.delete("/api/events/:eventId/venues/:venueId", async (req, res) => {
    try {
      ...
      if (allEventVenues.length <= 1) {
        return res
          .status(400)
          .json({ message: "Cannot remove last venue from event" });
      }
  ...
  app.get("/api/tables", async (_req, res) => {
    try {
      ...
      res.json([{ id: 1, tableNumber: 1, capacity: 4 }]);
    } catch (error) {
    ...
    try {
      ...
      eventBookings.forEach((booking) => {
        ...
        const seatNumbers =
          booking.seatNumbers ||
          (booking.partySize
            ? Array.from({ length: booking.partySize || 1 }, (_, i) => i + 1)
            : [1]);
  ...
  app.get("/api/tables/:tableId/seats", async (req, res) => {
    try {
      ...
      if (isNaN(eventId) || isNaN(tableId)) {
        return res
          .status(400)
          .json({ message: "Invalid table ID or event ID" });
      }

      console.log(
        `Getting seat availability for table ${tableId}, event ${eventId}`,
      );
  ...
  app.post("/api/bookings/:id/check-in", async (req, res) => {
    try {
      ...
      if (
        !req.isAuthenticated() ||
        !["admin", "venue_manager", "staff", "hostess"].includes(req.user?.role)
      ) {
        console.log(
          `SECURITY VIOLATION: Unauthorized check-in attempt - auth: ${req.isAuthenticated()}, role: ${req.user?.role}`,
        );
        return res.status(401).json({ message: "Unauthorized" });
      }
      ...
      if (!eventId || isNaN(eventId)) {
        console.log(
          `SECURITY VIOLATION: Check-in attempted for booking ${bookingId} without valid event ID: ${req.query.eventId}`,
        );
        return res.status(400).json({
      ...
      console.log(
        `SECURITY CHECK: Processing check-in for booking ${bookingId} by staff ${req.user.id} for event ${eventId}`,
      );
      ...
      if (!existingBooking) {
        console.log(
          `SECURITY CHECK: Booking ${bookingId} not found in database`,
        );
        return res.status(404).json({ message: "Booking not found" });
      }

      console.log(
        `SECURITY CHECK: Booking ${bookingId} belongs to event ${existingBooking.eventId}, requested event ${eventId}`,
      );

      // SECURITY: CRITICAL - Verify that the booking belongs to the specified event
      if (existingBooking.eventId !== eventId) {
        console.log(
          `SECURITY VIOLATION: Booking ${bookingId} is for event ${existingBooking.eventId}, but check-in attempted for event ${eventId}`,
        );
        console.error(
          `CRITICAL SECURITY BREACH: Cross-event check-in detected!`,
        );
        console.error(
          `BLOCKING UNAUTHORIZED ACCESS: Booking event ${existingBooking.eventId} != Selected event ${eventId}`,
        );
        return res.status(400).json({
      ...
      if (existingBooking.checkedIn) {
        console.log(
          `SECURITY VIOLATION: Duplicate check-in attempted for booking ${bookingId}`,
        );
        console.error(`CRITICAL SECURITY BREACH: Duplicate check-in detected!`);
        console.error(
          `BLOCKING DUPLICATE ACCESS: Booking ${bookingId} already checked in at ${existingBooking.checkedInAt}`,
        );
        return res.status(400).json({
      ...
      if (!event) {
        console.log(
          `SECURITY CHECK: Event ${existingBooking.eventId} not found`,
        );
        return res.status(404).json({ message: "Event not found" });
      }
      ...
      const now = new Date();
      const oneDayAfterEvent = new Date(
        eventDate.getTime() + 24 * 60 * 60 * 1000,
      );

      if (now > oneDayAfterEvent) {
        console.log(
          `SECURITY VIOLATION: Check-in attempted for past event ${existingBooking.eventId}`,
        );
        return res.status(400).json({
      ...
      if (existingBooking.status !== "confirmed") {
        console.log(
          `SECURITY VIOLATION: Booking ${bookingId} has status "${existingBooking.status}" and cannot be checked in`,
        );
        return res.status(400).json({
      ...
      console.log(
        `SECURITY PASSED: Proceeding with check-in for booking ${bookingId} in event ${eventId}`,
      );
      const updatedBooking = await storage.checkInBooking(
        bookingId,
        req.user.id,
      );
      if (!updatedBooking) {
        console.log(
          `CRITICAL ERROR: Failed to update booking ${bookingId} in database`,
        );
        return res.status(500).json({ message: "Failed to check in booking" });
      }

      console.log(
        `CHECK-IN SUCCESS: Booking ${bookingId} checked in at ${updatedBooking.checkedInAt}`,
      );
  ...
  app.get("/api/admin-logs/entity/:entityType", async (req, res) => {
    try {
      ...
      console.error(
        `Error fetching admin logs for entity type ${req.params.entityType}:`,
        error,
      );
      res.status(500).json({ message: "Failed to fetch admin logs" });
    }
  ...
  app.post("/api/manual-booking", async (req, res) => {
    try {
      ...
      console.log(
        "Received manual booking request:",
      ...
      console.log(
        "Creating manual booking with data:",
        JSON.stringify(bookingData, null, 2),
      );
      ...
      for (const event of allEvents) {
        ...
        if (eventBookings.length === 0) {
          ...
          const bookedSeats = eventBookings.reduce(
            (total, booking) => total + (booking.seatNumbers?.length || 0),
            0,
          );
  ...
  app.post("/api/events/order", async (req, res) => {
    try {
      ...
      res
        .status(200)
        .json({ success: true, message: "Events order updated successfully" });
    } catch (error) {
  ...
  app.post("/api/food-options/order", async (req, res) => {
    try {
      ...
      res
        ...
        .json({
          success: true,
          message: "Food options order updated successfully",
        });
    } catch (error) {
  ...
  // Update pricing tier
  app.put(
    "/api/events/:eventId/pricing-tiers/:tierId",
    async (req: any, res: any) => {
      try {
        ...
        if (isNaN(eventId) || isNaN(tierId)) {
          return res
            .status(400)
            .json({ message: "Invalid event ID or tier ID" });
        }

        const validationResult = insertEventPricingTierSchema
          .partial()
          .safeParse(req.body);
        if (!validationResult.success) {
          return res.status(400).json({
    ...
    async (req: any, res: any) => {
      try {
        ...
        if (assignedTables.length > 0) {
          return res.status(400).json({
            message:
              "Cannot delete pricing tier with assigned tables. Please reassign tables first.",
          });
        }
  ...
  app.get("/api/events/:eventId/table-assignments", async (req, res) => {
    try {
      ...
      const assignments = await db
        ...
        .leftJoin(
          eventPricingTiers,
          eq(eventTableAssignments.pricingTierId, eventPricingTiers.id),
        )
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch table assignments" });
    }
  });
  ...
  app.post("/api/admin/sync-refunds", async (req, res) => {
    try {
      ...
      for (const booking of bookingsWithPayments) {
        try {
          console.log(
            `Checking booking #${booking.id} (table ${booking.tableId}) with payment ID: ${booking.stripePaymentId}`,
          );
          ...
          const totalRefunded = charges.reduce(
            (sum: number, charge: any) => sum + (charge.amount_refunded || 0),
            0,
          );

          console.log(
            `Payment intent for booking #${booking.id}: amount_refunded=${totalRefunded}`,
          );
          ...
          if (totalRefunded > 0) {
            ...
            if (
              ...
              console.log(
                `Found unsynced refund for booking #${booking.id}: $${totalRefunded / 100} (current status: ${booking.status})`,
              );

              // Update booking with refund information
              await storage.updateBookingRefund(
                booking.id,
                totalRefunded,
                "stripe_sync",
              );
              ...
              console.log(
                `Refund amount discrepancy for booking #${booking.id}: DB has $${booking.refundAmount / 100}, Stripe has $${totalRefunded / 100}`,
              );
              await storage.updateBookingRefund(
                booking.id,
                totalRefunded,
            ...
            console.log(
              `Warning: Booking #${booking.id} marked as refunded in DB but not in Stripe`,
            );
            discrepancyCount++;
          }
          syncedCount++;
          ...
          console.error(
            `Error syncing refund for booking #${booking.id}:`,
            error,
          );
        }
      }
      ...
      console.log(
        `Refund sync completed: ${syncedCount} bookings checked, ${refundCount} refunds synced, ${discrepancyCount} discrepancies found`,
      );
  ...
  app.get("/api/user/:userId/bookings", async (req, res) => {
    try {
      ...
      const userBookings = Array.isArray(allBookings)
        ? allBookings.filter((booking: any) => booking.userId === userId)
        : [];
      res.json(userBookings);
    } catch (error) {
      console.error("Error fetching user bookings:", error);
  ...
  app.post("/api/bookings", async (req, res) => {
    ...
    try {
      ...
      const bookingData = {
        ...
        stripePaymentId:
          req.body.stripePaymentId || paymentMethod === "direct"
            ? `direct-${Date.now()}-${userId}`
            : `direct-${Date.now()}-${userId}`,
  ...
  app.post("/api/create-booking", async (req, res) => {
    ...
    try {
      ...
      if (!req.isAuthenticated() && !userId) {
        console.log(
          "🔴 Authentication failed - no session and no userId in body",
        );
        return res.status(401).json({ message: "Unauthorized" });
      }

      console.log(
        "🟢 Authentication successful - proceeding with booking creation",
      );
      ...
      console.log(
        "Calculated partySize:",
        req.body.seatNumbers?.length || req.body.partySize || 2,
      );
      ...
      const bookingData = {
        ...
        stripePaymentId:
          ...
          `direct-${Date.now()}-${userId || req.user.id}`,
        guestNames: req.body.guestNames || [],
        foodSelections: req.body.foodSelections || [],
        status: "confirmed",
      };
      ...
      console.log(
        "✅ Final booking data (NO VALIDATION):",
        JSON.stringify(bookingData, null, 2),
      );
  ...
  app.get("/api/bookings/:id", async (req, res) => {
    try {
      if (
        !req.isAuthenticated() ||
        !["admin", "venue_owner", "venue_manager"].includes(
          req.user?.role || "",
        )
      ) {
        return res.status(401).json({ message: "Unauthorized" });
    ...
    try {
      ...
      if (
        ...
        return res
          ...
          .json({
            message:
              "Invalid request body. Required: tableId and seatNumbers array",
          });
      }
      ...
      const validationResult =
        await BookingValidation.validateTableReassignment(
          tableId,
    ...
    try {
      ...
      if (!foodSelections || typeof foodSelections !== "object") {
        return res
          .status(400)
          .json({
            message: "Invalid request body. Required: foodSelections object",
          });
      }
  ...
  app.post("/api/bookings/:id/release", async (req, res) => {
    try {
      ...
      const updatedBooking = await storage.releaseTableManually(
        bookingId,
        req.user.id,
        reason,
      );
  ...
  app.post("/api/bookings/:id/add-note", async (req, res) => {
    try {
      ...
      if (!note || typeof note !== "string" || note.trim() === "") {
        return res
          .status(400)
          .json({ message: "Invalid request body. Required: non-empty note" });
      }
  ...
  app.post("/api/bookings/:id/refund", async (req, res) => {
    try {
      if (
        !req.isAuthenticated() ||
        !["admin", "venue_owner"].includes(req.user?.role || "")
      ) {
        return res.status(401).json({ message: "Unauthorized" });
      }
      ...
      if (typeof amount !== "number" || amount <= 0) {
        return res
          .status(400)
          .json({ message: "Invalid request body. Required: positive amount" });
      }
      ...
      try {
        console.log(
          `🔄 Starting refund process for booking ${bookingId}, amount: $${amount}`,
        );
        console.log(`💳 Payment ID: ${booking.stripePaymentId}`);

        // Detect if this is a test payment based on payment intent ID pattern
        const isTestPayment =
          booking.stripePaymentId.includes("test") ||
          booking.stripePaymentId.startsWith("pi_3Ru") ||
          booking.stripePaymentId.startsWith("pi_3RT");
        console.log(
          `🔍 Payment type detected: ${isTestPayment ? "TEST" : "LIVE"}`,
        );
        ...
        if (isTestPayment) {
          ...
          console.log(
            `🔑 Test key available: ${!!testKey}, prefix: ${testKey?.substring(0, 12)}`,
          );
        ...
        const instanceIdentifier =
          stripeInstance === stripe
            ? "PRIMARY"
            : stripeInstance === stripeTest
              ? "TEST"
              : "UNKNOWN";
        console.log(
          `📊 Using Stripe instance: ${isTestPayment ? "TEST" : "LIVE"} mode -> ${instanceIdentifier} instance (${stripeInstance ? "initialized" : "NOT initialized"})`,
        );
        console.log(
          `🔍 Instance check: stripe === stripeTest: ${stripe === stripeTest}`,
        );
        console.log(
          `🔍 stripeTest exists: ${!!stripeTest}, stripe exists: ${!!stripe}`,
        );

        // Check if the appropriate Stripe instance is initialized
        if (!stripeInstance) {
          console.error(
            "❌ Appropriate Stripe instance is not initialized. Cannot process refund.",
          );
        ...
        console.log(
          `🔄 Creating Stripe refund for payment intent: ${booking.stripePaymentId}`,
        );
        ...
        await AvailabilitySync.syncEventAvailability(booking.eventId);
        console.log(
          `✅ Availability synced for event ${booking.eventId} after refund`,
        );
  ...
  app.post("/api/bookings/:id/cancel", async (req, res) => {
    try {
      ...
      const updatedBooking = await storage.cancelBooking(
        bookingId,
        req.user.id,
      );
      ...
      console.log(
        `🔄 Syncing availability for event ${originalBooking.eventId} after cancellation`,
      );
      try {
        const syncResult = await storage.syncEventAvailability(
          originalBooking.eventId,
        );
        console.log(
          `✅ Event ${originalBooking.eventId} availability synced: ${JSON.stringify(syncResult)}`,
        );
      } catch (syncError) {
        console.error(
          "Failed to sync availability after cancellation:",
          syncError,
        );
  ...
  app.post("/api/admin/sync-stripe-amounts", async (req, res) => {
    try {
      ...
      console.log(
        `📊 Found ${allBookings.length} total bookings with Stripe payment IDs`,
      );
      ...
      console.log(
        `📊 Breakdown: ${bookingsToSync.length} need sync, ${testBookings.length} test payments, ${alreadySynced.length} already synced`,
      );
      ...
      for (const booking of bookingsToSync) {
        try {
          console.log(
            `🔍 Syncing payment ${booking.stripe_payment_id} for booking ${booking.id}`,
          );
          ...
          if (paymentIntent && paymentIntent.status === "succeeded") {
            ...
            syncResults.synced++;
            console.log(
              `✅ Updated booking ${booking.id}: $${paymentIntent.amount / 100} (${paymentIntent.amount} cents)`,
            );
          } else {
      ...
      console.log(
        `📈 Sync complete: ${syncResults.synced} synced, ${syncResults.errors.length} errors`,
      );
  ...
  app.patch("/api/food-options/:id", async (req, res) => {
    try {
      ...
      if (!foodOption) {
        return res
          .status(404)
          .json({ message: "Food option not found after update" });
      }
      ...
      for (const key of allowedKeys) {
        if (
          key in req.body &&
          JSON.stringify(
            originalFoodOption[0][key as keyof (typeof originalFoodOption)[0]],
          ) !== JSON.stringify(foodOption[key as keyof typeof foodOption])
        ) {
          changes[key] = {
            from: originalFoodOption[0][
              key as keyof (typeof originalFoodOption)[0]
            ],
  ...
  app.post("/api/generate-payment-token", async (req, res) => {
    ...
    if (req.isAuthenticated && req.isAuthenticated() && req.user) {
      ...
      console.log(
        `Generated payment token for user ${req.user.id} (${req.user.email})`,
      );
    ...
    // This is used as a fallback when session cookies can't be sent due to CORS issues
      ...
      try {
        console.log(
          `Attempting non-credentialed token generation with email ${req.body.email} and user ID ${req.body.userId}`,
        );
        ...
        if (user && user.email === req.body.email) {
          console.log(
            `Verified user match for direct auth: ${user.id} (${user.email})`,
          );
    ...
    // Tertiary authentication flow: email-only lookup
      ...
      try {
        ...
        if (user) {
          console.log(
            `Found user by email backup method: ${user.id} (${user.email})`,
          );
          ...
          console.log(
            `Generated limited payment token for user ${user.id} via email fallback`,
          );
    ...
    console.log(
      "Payment token request rejected: Not authenticated through any method",
    );
  ...
  app.post("/api/create-payment-intent", async (req, res) => {
    try {
      ...
      if (
        paymentToken &&
        app.locals.paymentTokens &&
        app.locals.paymentTokens[paymentToken]
      ) {
        ...
        if (tokenData.expires > Date.now()) {
          ...
          console.log(
            `Authentication via payment token for user ${tokenUser.id}`,
          );
        } else {
          // Token expired
      ...
      if (
        ...
        try {
          console.log(
            `Attempting direct authentication for user ID: ${req.body.userId}, email: ${req.body.userEmail}`,
          );
          ...
          if (user && user.email === req.body.userEmail) {
            ...
            console.log(
              `Direct authentication successful for user ${user.id} (${user.email})`,
            );
          } else {
            console.log(
              `Direct authentication failed - user mismatch or not found`,
            );
          }
      ...
      const user = isAuthenticatedViaSession
        ? req.user
        : isAuthenticatedViaToken
          ? tokenUser
          : directUser;
      ...
      if (!user) {
        console.error(
          "Critical error: User is null after authentication check",
        );
        return res.status(500).json({
      ...
      console.log(
        `Payment request authenticated for user: ${user.id} (${user.email})${isAuthenticatedViaSession ? `, session ID: ${req.sessionID.substring(0, 8)}...` : " via token"}`,
      );

      console.log(
        `Payment intent requested by user ${user.id} (${user.email})`,
      );
      ...
      if (!stripe) {
        console.error(
          "Stripe is not initialized. Attempting to initialize now...",
        );
        ...
        if (!initializeStripe() && stripeInitAttempt < MAX_INIT_ATTEMPTS) {
          ...
          if (!initializeStripe()) {
            console.error(
              `Failed to initialize Stripe after ${stripeInitAttempt} attempts`,
            );
      ...
      if (
        !req.body ||
        typeof req.body.seatCount !== "number" ||
        req.body.seatCount < 1
      ) {
        return res.status(400).json({
      ...
      console.log(
        `Creating payment intent for amount: ${amount} cents, user: ${user.id}, seats: ${seatCount}`,
      );
      ...
      try {
        ...
        console.error(
          "Stripe API error when creating payment intent:",
          stripeError,
        );
        ...
        if (stripeError.type === "StripeCardError") {
          ...
          errorMessage =
            "Payment service is experiencing technical difficulties. Please try again later.";
          errorCode = "API_ERROR";
        } else if (stripeError.type === "StripeConnectionError") {
          errorMessage =
            "Could not connect to payment service. Please check your internet connection and try again.";
    ...
    try {
      ...
      if (!stripe && process.env.STRIPE_SECRET_KEY) {
        try {
          ...
          diagnostics.stripeInstance.apiVersion =
            (stripe as any).apiVersion || stripeApiVersion;
        } catch (initError: any) {
      ...
      if (stripe) {
        ...
        try {
          ...
          diagnostics.tests.authentication = {
            ...
            endTime: new Date(endTime).toISOString(),
            durationMs:
              endTime - Date.parse(diagnostics.tests.authentication.startTime),
            error: {
      ...
      res.json({
        ...
        overall: {
          ...
          ready:
            diagnostics.stripeInstance.initialized &&
  ...
  app.get("/api/venue-layout", async (req, res) => {
    try {
      ...
      const event = events.find(
        (e: any) => e.id === parseInt(eventId as string),
      );
      if (!event) {
        return res.status(404).json({ message: "Event not found" });
      }
      ...
      const venueTables = tables.filter(
        (t: any) => t.venue_id === venueId || t.venueId === venueId,
      );
  ...
  app.get("/api/events/:eventId/venue-layouts", async (req, res) => {
    try {
      ...
      const eventVenuesList = await db
        ...
        .leftJoin(venues, eq(eventVenues.venueId, venues.id))
        .where(
          and(eq(eventVenues.eventId, eventId), eq(eventVenues.isActive, true)),
        )
        .orderBy(eventVenues.displayOrder);
      ...
      if (eventVenuesList.length === 0) {
        return res
          .status(404)
          .json({ error: "No active venues found for this event" });
      }
      ...
      const venueLayouts = await Promise.all(
        eventVenuesList.map(async (eventVenue) => {
          ...
          console.log(
            `🔍 SERVER: Fetching tables for venue ${venueId} (${eventVenue.displayName})`,
          );

          // CRITICAL FIX: Get tables with real-time booking status calculation
          console.log(
            `🚨 CRITICAL: Getting real-time table status for venue ${venueId}, event ${eventId}`,
          );
          const venueTables = await storage.getTablesByVenue(venueId, eventId);

          console.log(
            `📊 SERVER: Found ${venueTables.length} tables for venue ${venueId}`,
          );
          console.log(
            `🔍 CRITICAL STATUS CHECK:`,
            venueTables
              .filter((t) => [11, 16].includes(t.tableNumber))
              .map((t) => ({ num: t.tableNumber, status: t.status })),
          );
  ...
  app.get("/api/event-bookings", async (req, res) => {
    try {
      ...
      const bookings = await storage.getBookingsByEventId(
  ...
  app.get("/api/download-ticket/:bookingId", async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="ticket-${booking.id}-${event.title.replace(/[^a-zA-Z0-9]/g, "-")}.pdf"`,
      );
      ...
      doc
        .fontSize(24)
        .fillColor("#2c3e50")
        .text("The Treasury 1929", { align: "center" });
      doc
        .fontSize(18)
        .fillColor("#27ae60")
        .text("Digital Ticket", { align: "center" });
      doc.moveDown(1);
      ...
      doc
        .fontSize(14)
        .fillColor("#495057")
        .text(eventDateFormatted, { align: "center" });
      doc
        .fontSize(12)
        .fillColor("#6c757d")
        .text(timeDisplay, { align: "center" });
      doc.moveDown(1);
      ...
      doc
        .fontSize(12)
        .fillColor("#666")
        .text("Scan this QR code at the venue for quick check-in", {
          align: "center",
        });
      doc.moveDown(1);
    ...
    try {
      ...
      if (!stripe) {
        ...
        return res.status(500).json({
          ...
          error:
            "Payment service unavailable. Please check environment variables.",
        });
      }
