...
import {
  ...
  type Venue,
  type Stage,
  type VenueWithTables,
} from "@shared/schema";
import type { IStorage } from "./storage-base.js";
import { eq, and, desc, asc, gte, lt, sql, or } from "drizzle-orm";
...
export class PgStorage implements IStorage {
  ...
  async updateUserDietaryPreferences(
    userId: number,
    allergens: string[],
  ...
  async getStagesByVenue(venueId: number): Promise<Stage[]> {
    return await db
      ...
      .where(
        and(
          eq(schema.stages.venueId, venueId),
          eq(schema.stages.isActive, true),
        ),
      );
  }
  ...
  async getAllEvents(): Promise<Event[]> {
    return await db
      .select()
      .from(schema.events)
      .orderBy(asc(schema.events.displayOrder), desc(schema.events.date));
  }
  ...
  async getInactiveEvents(): Promise<Event[]> {
    ...
    return await db
      ...
      .from(schema.events)
      .where(
        or(eq(schema.events.isActive, false), lt(schema.events.date, today)),
      )
  ...
  async getBookingByPaymentId(paymentId: string): Promise<Booking | null> {
    const result = await db
      .select()
      .from(schema.bookings)
      .where(eq(schema.bookings.stripePaymentId, paymentId));
    return result[0] || null;
  }

  async createBooking(bookingData: any): Promise<Booking> {
    console.log(
      "🔍 STORAGE DEBUG: About to insert booking data:",
  ...
  async getAvailableTablesByEventId(eventId: number): Promise<Table[]> {
    ...
    const bookedTables = await db
      .select({ tableId: schema.bookings.tableId })
      .from(schema.bookings)
      .where(
        and(
  ...
  async releaseTableManually(
    ...
    const result = await db
      ...
      .set({
        ...
        modifiedBy,
        notes: booking.notes
          ? `${booking.notes} | Manual release: ${reason || "No reason provided"}`
          : `Manual release: ${reason || "No reason provided"}`,
      })
      .where(eq(schema.bookings.id, bookingId))
      .returning();
  ...
  async getTablesByVenue(venueId: number, eventId?: number): Promise<Table[]> {
    if (eventId) {
      console.log(
        `🚨 CRITICAL: Real-time table status for venue ${venueId}, event ${eventId}`,
      );
      ...
      const tablesWithStatus = await Promise.all(
        baseTables.map(async (table) => {
          ...
          if (confirmedBooking.length > 0) {
            console.log(
              `🔴 Table ${table.tableNumber} is BOOKED (booking: ${confirmedBooking[0].id})`,
            );
            return { ...table, status: "booked" as const };
          }

          // Check for active seat holds
          const activeSeatHold = await db
            .select()
          ...
          if (activeSeatHold.length > 0) {
            console.log(
              `🟠 Table ${table.tableNumber} is on HOLD (hold: ${activeSeatHold[0].id})`,
            );
  ...
  async getTablesByVenueAndFloor(
    venueId: number,
    floor: string,
  ): Promise<Table[]> {
    return await db
      ...
      .from(schema.tables)
      .where(
        and(eq(schema.tables.venueId, venueId), eq(schema.tables.floor, floor)),
      );
  }
  ...
  async getTableSeats(tableId: number): Promise<Seat[]> {
    return await db
      .select()
      .from(schema.seats)
      .where(eq(schema.seats.tableId, tableId));
  }
  ...
  async getMenuCategories(): Promise<string[]> {
    const result = await db
      .selectDistinct({ category: schema.menuItems.category })
  ...
  async getVenueStaff(): Promise<VenueStaff[]> {
    return await db
      ...
      .where(eq(schema.venueStaff.isActive, true));
  }
  ...
  async getStaffMember(id: number): Promise<VenueStaff | null> {
    const result = await db
      .select()
      .from(schema.venueStaff)
      .where(eq(schema.venueStaff.id, id));
    return result[0] || null;
  }
  ...
  async createStaffMember(staffData: any): Promise<number> {
    const result = await db
      .insert(schema.venueStaff)
      .values(staffData)
      .returning({ id: schema.venueStaff.id });
    return result[0].id;
  }

  async updateStaffMember(
    id: number,
    staffData: Partial<VenueStaff>,
  ): Promise<boolean> {
    const result = await db
      .update(schema.venueStaff)
      .set(staffData)
      .where(eq(schema.venueStaff.id, id));
    return (result.rowCount ?? 0) > 0;
  }
  ...
  async getEventFoodOptions(eventId: number): Promise<any[]> {
    const result = await db
      ...
      .innerJoin(
        schema.foodOptions,
        eq(schema.eventFoodOptions.foodOptionId, schema.foodOptions.id),
      )
      .where(
        and(
  ...
  async getFoodOptionsByIds(ids: number[]): Promise<any[]> {
    return await db
      .select()
      .from(schema.foodOptions)
      .where(eq(schema.foodOptions.id, ids[0]));
  }

  async createFoodOption(foodData: any): Promise<any> {
    const result = await db
      .insert(schema.foodOptions)
      .values(foodData)
      .returning();
    return result[0];
  }
  ...
  async updateFoodOption(id: number, foodData: any): Promise<any> {
    const result = await db
      .update(schema.foodOptions)
      .set(foodData)
      .where(eq(schema.foodOptions.id, id))
      .returning();
    return result[0];
  }
    ...
    const detailedOrders = bookings.map((booking) => {
      ...
      if (booking.orderTracking) {
        try {
          ...
          console.error(
            "Error parsing order tracking for booking",
            booking.id,
            e,
          );
        }
      ...
      if (
        processedFoodSelections.length === 0 &&
        booking.foodSelections &&
        Array.isArray(booking.foodSelections)
      ) {
          ...
          if (selection && typeof selection === "object") {
            ...
            if (booking.guestNames) {
              if (Array.isArray(booking.guestNames)) {
                ...
                guestName =
                  booking.guestNames[guestKey] || `Guest ${index + 1}`;
              }
            }
